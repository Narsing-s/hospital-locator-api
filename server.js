...
// GET /api/pincode?pincode=531034
...
app.post("/api/patients", async (req, res) => {
  try {
    ...
    // Normalize inbound to internal canonical
    const payload = {
    ...
    // Map to Mule-required keys (CASE SENSITIVE)
...
<style>
  :root { --bg1:#1e3c72; --bg2:#2a5298; --accent:#00c6ff; --accent2:#0072ff; --text:#fff; --mono:#00ff90; --panel:#000; }
  body{ font-family:Segoe UI, Arial, sans-serif; background:linear-gradient(135deg, var(--bg1), var(--bg2)); color:var(--text); text-align:center; padding:20px; }
  .container{ max-width:980px; margin:0 auto; }
  .row{ margin:22px 0; }
  input,button{ padding:10px; margin:6px 5px; border-radius:8px; border:none; }
  input{ min-width:260px; }
  button{ background:var(--accent); color:#fff; cursor:pointer; }
  button:hover{ background:var(--accent2); }
  pre{ background:var(--panel); color:var(--mono); padding:15px; border-radius:10px; text-align:left; overflow:auto; max-height:55vh; }
  .grid{ display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:12px; }
  fieldset{ border:1px solid rgba(255,255,255,0.15); border-radius:12px; padding:12px; min-height:160px; }
